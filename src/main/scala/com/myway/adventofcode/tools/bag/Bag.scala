...
object BagOps {
  ...
  def codomain[K, V](map: Map[K, V]): Bag[V] =
    map.keySet.foldLeft[Bag[V]](new Bag())((bg, k) => bg.add(map(k)))

  def ix[K, V](bag: Bag[(K, V)]): Map[K, Bag[V]] =
    bag.bagMap.keySet.foldLeft[Map[K, Bag[V]]](Map())((mp, kv) =>
      mp +
        (kv._1 ->
          new Bag(kv._2, bag.get(kv)).merge(mp.getOrElse(kv._1, new Bag())))
    )

  def ix0[K, V](map: Map[K, Bag[V]]): Bag[(K, V)] =
    map.keySet.foldLeft[Bag[(K, V)]](new Bag())((bg, k) =>
      map.get(k).map(_.map(v => (k, v))).get.merge(bg)
    )
