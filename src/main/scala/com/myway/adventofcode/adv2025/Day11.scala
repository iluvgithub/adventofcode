...
object Day11 {
  ...
  def loop(map: Map[String, List[String]], tgt: String, bag: Bag[Trajectory], nbStp: Int): Long = {
    ...
    val next: List[(Trajectory, String)] =
      keys.toList.flatMap(tr => ListUtil.cpl(tr, map.getOrElse(tr.location, Nil)))
  ...
  def parseMap(data: List[String]): Map[String, List[String]] =
    data.map(parse).map(d => d.from -> d.to).toMap
