...
object Day10 {
  ...
  def main(args: Array[String]): Unit = {
    ...
    println(solve1(data))
    println(solve2(data)) // 16855 too low, 17209 too low 17842 too low, 17846 not good, 17850 ??? 17856/
  }
  ...
  case class Machine(light: List[Boolean], buttons: List[Button], joltage: List[Int]) {
    ...
    def solve2b: Long = {
      ...
      val map: Map[Joltage, Int] = buttons.map(new Joltage(_, sz)).map(j => j -> 1).toMap ++ Map(empty -> 0)
      loop(map, target)
    }

    def loop(map: Map[Joltage, Int], target: Joltage): Long = if (map.contains(target)) map(target).toLong else {
  ...
  case class Joltage(joltage: List[Int]) {
    ...
    def add(that: Joltage): Joltage = Joltage(this.joltage.zip(that.joltage).map({ case (u, v) => u + v }))
  ...
  case class Button(ids: List[Int]) {
    ...
    def toArray(sz: Int): Array[Int] = List.range(0, sz)
  ...
  def gjr(a: List[List[Int]], b: List[Int]): (List[List[Double]], List[Double]) = {
    ...
    val a1 = (for (i <- 0 until m) yield
      (for (j <- 0 until n) yield mat(i)(j)).toList
      ).toList
  ...
  def solveUpperTriangular(t: List[List[Double]], b: List[Double]): List[Double] = {
    ...
    if (b.length != n || t.exists(_.length != n)) {
      throw new IllegalArgumentException(s"Matrix must be square and b must match dimension, n=$n, b:${b.length}")
    }
    ...
    while (i >= 0) {
      ...
      if (math.abs(diag) < 1e-12) {
        throw new ArithmeticException(s"Zero or near-zero on diagonal at position ($i,$i): cannot solve")
      }
...
object TriangularSolver {
  ...
  /**
   * Solves the linear system T * x = b where T is an upper triangular matrix
   * (i > j implies T(i)(j) == 0, 0-based indexing) with positive diagonal entries.
   *
   * This version uses immutable Lists in the public signature for input/output,
   * but internally converts to Arrays for efficient random access during solving.
   *
   * @param t Upper triangular matrix as List[List[Double]] (n x n)
   * @param b Right-hand side vector as List[Double] (length n)
   * @return Solution vector x as List[Double]
  ...
  def solveUpperTriangular(t: List[List[Double]], b: List[Double]): List[Double] = {
    ...
    val n = t.size
    if (b.size != n) {
      throw new IllegalArgumentException(s"Dimension mismatch: matrix is ${n}x$n, vector length ${b.size}")
    }
  ...
  def solve(a: List[List[Int]], b0: List[Int], t: List[List[Double]], b: List[Double]): List[Int] = {
    val nbCols = t(1).size
    val idx: List[Int] = List.range(0, nbCols).foldLeft[(Int, List[Int])]((0, Nil))(
      (acc, j) => if (acc._1 >= t.length) acc else if (Math.abs(t(acc._1)(j)) > 0) (acc._1 + 1, j :: acc._2) else acc
    )._2.reverse
    val tri: List[List[Double]] = idx.zipWithIndex.foldLeft[List[List[Double]]](Nil)(
      (mx, x) => {
        val j = x._1
        val i = x._2
    ...
    val o: List[List[(Int, Long)]] = diff.map(x => ListUtil.cpl(x, List.range(0, computeMax(x)+1).map(_.toLong)))
    val cpList: List[List[(Int, Long)]] = ListUtil.cpList(o)
    val oo = cpList.map(l => {

        val b1 = b.take(tri.length).zipWithIndex.
          map({ case (x, i) => l.foldLeft(x)((acc, t2) => acc - t2._2 * t(i)(t2._1)) })

        val solution: List[Double] = solveUpperTriangular(tri, b1)
        val complete = List.range(0, nbCols).foldLeft[(List[Long], Int, Int, Double)]((Nil, 0, 0, 0.0))((acc, j) => {
          if (idx.contains(j)) {
  ...
  def mult(a: List[List[Int]], x: List[Int], b: List[Int]): Int = {
    val b1 = List.range(0, a.length).map(
      i => List.range(0, x.size).foldLeft(0)((acc, j) => acc + a(i)(j) * x(j)) - b(i)
    )
    b1.sum
  }
