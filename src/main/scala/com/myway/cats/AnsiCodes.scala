package com.myway.cats

import cats.data.State
import com.myway.cats.Terminal.Program
object AnsiCodes {
  ...
  def bold[A](program: Program[A]): Program[A] =
    for {
      _ <- State.modify[Terminal] { terminal =>
        if (terminal.bold == 0) Console.print(AnsiCodes.boldOn)
        terminal.boldOn
      }
      a <- program
      _ <- State.modify[Terminal] { terminal =>
        val newTerminal = terminal.boldOff
        if (terminal.bold == 0 ) Console.print(AnsiCodes.boldOff)
        newTerminal
      }
    } yield a
  ...
  def main(args:Array[String]): Unit = {
    val program =
      for {
        _ <- Program.blue(
          for {
            _ <- Program.print("This is blue ")
            _ <- Program.red(Program.print("and this is red "))
            _ <- Program.bold(Program.print("and this is blue and bold "))
          } yield ()
        )
        _ <- Program.print("and this is back to normal.\n")
      } yield ()
    Program.run(program)
  }
}