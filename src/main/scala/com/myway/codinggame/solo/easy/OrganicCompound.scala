...
object OrganicCompound {
  ...
  case class CarboUnit(nbHydro: Int)
  case class Molecule(head: CarboUnit, tail: List[(Int, CarboUnit)]) {
    def isValid: Boolean = if (tail.isEmpty) {
      4 == head.nbHydro
    } else {
      val last: (Int, CarboUnit)  = tail.takeRight(1).head
      val first: (Int, CarboUnit) = (tail.take(1).head._1, head)
      val b      = (4 == last._1 + last._2.nbHydro) && (first._1 + first._2.nbHydro == 4)
      val liaise = tail.map(_._1)
      val zip: Seq[(Int, Int)] = liaise.zip(liaise.tail)
      val rights               = tail.map(_._2).dropRight(1)
      b && rights.zip(zip).forall { case (cu, (l, r)) => l + cu.nbHydro + r == 4 }
    }

  }

  val parseCarbo: Parser[CarboUnit] = for {
    _ <- Parser.char0('C')
    _ <- Parser.char0('H')
    n <- Parser.number.map(_.toInt)
  } yield CarboUnit(n)

  val pairParser: Parser[(Int, CarboUnit)] = for {
    _  <- Parser.char0('(')
    n  <- Parser.number.map(_.toInt)
    _  <- Parser.char0(')')
    cu <- parseCarbo
  ...
  def molParser: Parser[Molecule] = for {
    cu  <- parseCarbo
    lst <- Parser.many(pairParser)
  } yield Molecule(cu, lst)

  def isValid(s: String): Boolean =
    molParser.parseSure(s).isValid

  private def isValidList(l: List[String]) =
    l.tail.forall(isValid)

  def solve(ls: List[String]): String = if (isValidList(ls)) "VALID" else "INVALID"
